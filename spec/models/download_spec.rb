...
RSpec.describe Download, type: :model do
  ...
  describe 'defaults' do
    it 'sets default status to queued' do
      download = Download.new(user: user, chat: chat, url: 'http://example.com')
      expect(download.status).to eq('queued')
    end

    it 'sets default audio_only to false if not specified (via DB/Model)' do
      download = Download.create!(user: user, chat: chat, url: 'http://example.com')
  ...
  describe 'soft delete scopes' do
    it 'excludes soft-deleted records from default scope' do
      active_download = Download.create!(user: user, chat: chat, url: 'http://example.com/active')
      deleted_download = Download.create!(user: user, chat: chat, url: 'http://example.com/deleted')
