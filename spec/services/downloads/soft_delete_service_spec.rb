...
RSpec.describe Downloads::SoftDeleteService do
  ...
  describe '.by_user!' do
    it 'soft deletes all user downloads' do
      user_download_1 = Download.create!(user: user, chat_id: 1, url: 'https://example.com/1', audio_only: false)
      user_download_2 = Download.create!(user: user, chat_id: 1, url: 'https://example.com/2', audio_only: false)
      other_user = User.create!(telegram_user_id: 999, username: 'other')
      active_other = Download.create!(user: other_user, chat_id: 1, url: 'https://example.com/3', audio_only: false)
